...
function makeArrayWithNumbers(size, firstNumber) {
  ...
  for (var i = firstNumber; i < size + firstNumber; i++) array.push(i)
  return array
}
...
var ButtonGrid = {
  ...
  show: function (width, height, button_passport_list, props) {
    ...
    function makeArrayWithNumbersForUI(size, firstNumber) {
      return (
        makeArrayWithNumbers(size, firstNumber)
    ...
    var x_pos_list = makeArrayWithNumbersForUI(width).map(function (num) {
      return num * X_MARGIN
    })
    var y_pos_list = makeArrayWithNumbersForUI(height, -1).map(function (num) {
      return num * Y_MARGIN
    })
    ...
    for (var i in x_pos_list) {
      for (var j in y_pos_list) {
        ...
        if (current_button_passport == undefined) return
...
var TranslationList = {
  ...
  hide: function () {
    ;[100, 101].map(function (num) {
      return UI.removeComponent(num)
    })
  },
}
...
var ChapterList = {
  show: function () {
    ...
    ButtonGrid.show(8, 19, button_passports, {
      button_width: 24,
      x_margin: 28,
    })
  },
...
var UI
void (function (guiId) {
  scriptGuiList[guiId] = Java.type("noppes.npcs.api.NpcAPI")
    .Instance()
    .createCustomGui(guiId, 256, 256, false)
