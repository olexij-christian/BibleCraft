...
function interact(e) {
  ...
  if (e.type == 2) {
    if (selected_building) {
      ...
      if (
        first_block_pos &&
        first_block_pos.distanceTo(e.target.getPos()) == 0
      ) {
        // build
      ...
      } else second_block_pos = e.target.getPos()
    }
  }
...
function tick(e) {
  ...
  if (!first_block_pos || !second_block_pos) return
  ...
  if (e.item.compare(player_item, false)) {
    ...
    spawnParticleBy3DEdgeArea(
      e.player.world,
      "flame",
      first_block_pos,
      second_block_pos_for_particles
    )
  }
}
...
void (function (guiId) {
  ...
  scriptGuiList[guiId].setBackgroundTexture(
    "customnpcs:textures/gui/smallbg.png"
  )
  ...
  scriptGuiEvents.button[3] = function (event) {
    ...
    event.player.message(
      'Постойка успешно сохранена под названием "' + file_name + '"'
    )
